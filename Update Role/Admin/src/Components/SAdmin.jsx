...
const SAdmin = () => {
  ...
  const fetchProducts = () => {
    axios
      .get("http://localhost:8080/products", getAuthHeaders())
  ...
  const handleAddProduct = (e) => {
    ...
    if (editingProduct) {
      axios
        .put(`http://localhost:8080/products/${editingProduct._id}`, newProduct, getAuthHeaders())
        .then((res) => {
      ...
      axios
        .post("http://localhost:8080/products", newProduct, getAuthHeaders())
        .then((res) => {
          alert(res.data.message);
          fetchProducts();
          setNewProduct({ name: "", price: "", description: "" });
  ...
  const handleDeleteProduct = (productId) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      axios
        .delete(`http://localhost:8080/products/${productId}`, getAuthHeaders())
        .then((res) => {
          alert(res.data.message);
          fetchProducts();
        })
      ...
      <div className="w-full max-w-5xl bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl p-8 border border-white/20">
        ...
        {showProductForm && (
          <div className="mt-8 p-6 bg-white/5 rounded-lg border border-white/10">
            <h3 className="text-xl font-semibold mb-4 text-[#F48FB1]">
            ...
            <form onSubmit={handleAddProduct} className="space-y-4">
              ...
              <textarea
                ...
                className="w-full px-4 py-2 h-24 rounded-lg bg-[#2e2e2e] border border-gray-500 text-white"
              />
              <button
                type="submit"
                className="w-full px-6 py-2 rounded-lg bg-gradient-to-r from-purple-600 to-pink-600 text-white"
              >
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {products.map((product) => (
              <div key={product._id} className="p-4 bg-white/5 rounded-xl border border-white/10">
                <h4 className="text-lg text-[#81D4FA]">{product.name}</h4>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 px-3 py-1 bg-blue-600 rounded-lg"
                  >
                    Edit
                  </button>
                  ...
                  <button
                    ...
                    className="flex-1 px-3 py-1 bg-red-600 rounded-lg"
                  >
                    Delete
                  </button>
                </div>
          ...
          {products.length === 0 && (
            <p className="text-center text-gray-300 mt-8">No products found.</p>
          )}
        </div>
